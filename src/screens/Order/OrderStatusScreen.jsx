...
const OrderStatusScreen = () => {
  ...
  useEffect(() => {
    ...
    socket.on('orderStatusUpdate', ({ orderId, status, detailDriver }) => {
      ...
      console.log(status);
      if (status === 'ORDER_CONFIRMED') clearRoutesFromStorage();
    });
      ...
      <View View style={styles.orderInfoContainer}>
        ...
        <View style={styles.progressContainer}>
          <View
            style={[
              styles.progressItem,
              orderStatus === 'PAID' && styles.activeStep,
            ]}>
            <Ionicons
              name="checkmark-circle-outline"
              size={24}
              color={orderStatus === 'PAID' ? '#007AFF' : '#9E9E9E'}
            />
          ...
          <TouchableOpacity
            style={[
              styles.progressItem,
              (orderStatus === 'DELIVERING' && styles.activeStep) ||
                (orderStatus === 'PREPARING_ORDER' && styles.activeStep),
            ]}>
            <Ionicons
              name="fast-food-outline"
              size={24}
              color={orderStatus === 'DELIVERING' ? '#007AFF' : '#9E9E9E'}
            />
          ...
          <TouchableOpacity
            style={[
              styles.progressItem,
              orderStatus === 'ORDER_RECEIVED' && styles.activeStep,
            ]}>
            <Ionicons
              name="bicycle-outline"
              size={24}
              color={orderStatus === 'ORDER_RECEIVED' ? '#007AFF' : '#9E9E9E'}
            />
          ...
          <TouchableOpacity
            style={[
              styles.progressItem,
              orderStatus === 'ORDER_CONFIRMED' && styles.activeStep,
            ]}>
            <Ionicons
              name="checkmark-done-circle-outline"
              size={24}
              color={orderStatus === 'completed' ? '#007AFF' : '#9E9E9E'}
            />
